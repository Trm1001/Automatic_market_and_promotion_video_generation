...
from utils import ( create_logger, update_ema,
...
class Mapper(nn.Module):
    def __init__(self,
        ...
        for i in range(5):
            setattr(self, f'mapping_{i}', nn.Sequential(nn.Linear(input_dim, 512),
                                                        nn.LayerNorm(512),
                                                        nn.LeakyReLU(),
                                                        nn.Linear(512, 512),
                                                        nn.LayerNorm(512),
            ...
            setattr(self, f'mapping_patch_{i}', nn.Sequential(nn.Linear(input_dim, 512),
                                                              nn.LayerNorm(512),
                                                              nn.LeakyReLU(),
                                                              nn.Linear(512, 512),
                                                              nn.LayerNorm(512),
...
def validation(diffusion, unet, image_encoder, tokenizer, video_data, text_encoder, vae, cfg_scale, device, save_dir):
    ...
    encoder_hidden_states_con = text_encoder({'input_ids': original_ids})[0]
...
def main(args):
    ...
    # 创建日志记录器
    logger = create_logger(log_dir, log_file)
    ...
    state_dict = torch.load(args.pretrained_t2v_model,  map_location=lambda storage, loc: storage.cuda(torch.cuda.current_device()))["ema_unet"]

    # 打印ema_unet的键
    print("Keys in ema_unet:", state_dict.keys())

    unet.load_state_dict(state_dict, strict=False)

    # 对 unet 模型进行动态量化
    # unet = quantize_dynamic(unet, {torch.nn.Linear, torch.nn.Conv3d}, dtype=torch.qint8)
    ...
    opt = torch.optim.AdamW(unet.parameters(), lr=1e-4, weight_decay=0)
    ...
    unet.train()  # important! This enables embedding dropout for classifier-free guidance
    ema = deepcopy(unet).to(device)  # Create an EMA of the model for use after training
    ...
    for epoch in range(args.epochs):
        ...
        logger.info(f"Starting Epoch {epoch + 1}/{total_epochs}, remaining_epochs:{remaining_epochs}")
        sampler.set_epoch(epoch)
        print(f"Starting Epoch {epoch + 1}/{total_epochs}, remaining_epochs:{remaining_epochs}")
        for i, video_data in enumerate(loader):
            logger.info(f"Processing batch {i + 1} of epoch {epoch + 1}")
            try:
                x = video_data['video'].to(device)
                video_name = video_data['video_name']
                logger.info("Data loaded and moved to device")

                # 在不影响梯度的情况下进行预处理和特征抽取
                with torch.no_grad():
                    b, _, _, _, _ = x.shape
                    x = rearrange(x, 'b f c h w -> (b f) c h w').contiguous()
                    x = vae.encode(x).latent_dist.sample().mul_(0.18215)
                    x = rearrange(x, '(b f) c h w -> b c f h w', b=b).contiguous()  # for tav unet; b c f h w is for conv3d
                    logger.info("Data rearranged and encoded")

                # TODO: why should we resize again?
                image = F.interpolate(video_data["masked_first_frame"].to(device), (224, 224), mode='bilinear')
                image = normalize_exemplar(image)
                logger.info("First frame interpolated and normalized")

                # get the image embeddings
                image_features = image_encoder(image, output_hidden_states=True)
                image_embeddings = [image_features[0], image_features[2][4], image_features[2][8], image_features[2][12], image_features[2][16]]
                image_embeddings = [emb.detach() for emb in image_embeddings]
                logger.info("Image embeddings extracted")
                ...
                encoder_hidden_states = text_encoder({'input_ids': original_ids})[0]
                logger.info("Text features extracted")

                t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=device)
                model_kwargs = dict(encoder_hidden_states=encoder_hidden_states, class_labels=None)

                loss_dict = diffusion.training_losses(unet, x, t, model_kwargs)
                loss_ldm = loss_dict["loss"].mean()
                loss_reg = torch.mean(torch.abs(image_embeddings)) * args.reg_weight
                loss = loss_ldm + loss_reg
                logger.info("Loss computed")

                opt.zero_grad()
                loss.backward()
                logger.info("Backpropagation done")

                if args.clip_max_norm:
                    gradient_norm = clip_grad_norm_(unet.parameters(), args.clip_max_norm, clip_grad=True)
                    logger.info(f"Gradients clipped to {args.clip_max_norm}")

                opt.step()
                update_ema(ema, unet)
                logger.info("Optimizer and EMA updated")
                ...
                if train_steps % args.log_every == 0:
                    ...
                    logger.info(f"(step={train_steps:07d}/epoch={epoch:04d}) Train Loss: {avg_loss:.4f}, Loss ldm: {avg_loss_ldm:.4f}, Loss reg: {avg_loss_reg:.4f}, Gradient Norm: {gradient_norm:.4f}, Train Steps/Sec: {steps_per_sec:.2f}")
                ...
                if train_steps % args.ckpt_every == 0 and train_steps > 0:
                    if rank == 0:
                        ...
                        validation(diffusion, unet, image_encoder, tokenizer, video_data, text_encoder, vae, args.cfg_scale, device, f'{experiment_dir}/validation/iters_{train_steps}')
                    dist.barrier()

            except Exception as e:
                logger.error(f"An error occurred at batch {i + 1} of epoch {epoch + 1}: {str(e)}")
                break

    unet.eval()  # important! This disables randomized embedding dropout
